<?php
    ...
    class ffmpeg {
		...
		function __construct($ffmpeg = "", $ffprobe = "") {
			if(strtolower(PHP_OS) != "winnt" || strtolower(PHP_OS) != "windows") {
				$this->FFMPEG = "/usr/bin/ffmpeg";
				$this->FFPROBE = "/usr/bin/ffprobe";
			} else {
				$this->FFMPEG = ($ffmpeg != "" ? $ffmpeg : "/usr/bin/ffmpeg");
				$this->FFPROBE = ($ffprobe != "" ? $ffprobe : "/usr/bin/ffprobe");
			}
		}
